function [ output_args ] = plot_summary_dim_projections_iti( proj_a, proj_e, dim_ves, saveFigs,out_dir)

    n_dims = 5; % leave out PC2

    lims = [-2.5 7.5];
    cols = chooseColors;
    
    rep_mean_a = squeeze(mean(cat(3,proj_a{:}),3));
    rep_mean_e = squeeze(mean(cat(3,proj_e{:}),3));
    rep_mean_ve = squeeze(mean(cat(1,dim_ves{:}),1));


    %% plot ITI-shuttle and ITI rand (alignment like for trials)
    figure('Position',[100 100 1500 300])
    for nd = 1:n_dims
        subplot(1,n_dims,nd)
        hold on

        ylim(lims)
        xlim([0 46])
        set(gca,'XTick',[5 20 25 40]) 
        set(gca,'XTickLabel',[0 3 -3 0])
        xlabel('time (s)')
        plot([5,5],lims,'Color',[.7 .7 .7])
        plot([40,40],lims,'Color',[.7 .7 .7])

        % add sep for discontinuity
        sep = zeros(25,1)/0;
        a_plt = [sep ;rep_mean_a(6:25,nd)];
        e_plt = [rep_mean_e(6:25,nd); sep];

        plot(a_plt,'k','LineWidth',2)
        plot(e_plt,'Color',cols(5,:),'LineWidth',2)
        if nd == 1
            ylabel('projection (a.u.)')
        end
    end
    
    if saveFigs
        fpath = fullfile(out_dir,'fig4','coding_dim_projections_iti.svg');
        saveas(gca,fpath,'svg')
    end
    
    %% plot distribution of VEs
    % exchange PC2 var with 'residual var'
    dim_ves_total = dim_ves;
    for rep = 1:length(dim_ves_total)
        dim_ves_total{rep}(6) = 100 - sum(dim_ves_total{rep}(1:5));
    end
    
    rep_ves = cat(1,dim_ves_total{:});
    rep_ves = rep_ves(:,1:6);

    mean_total_ves = mean(rep_ves);
    std_total_ves = std(rep_ves);
    
    this_cols{1} = [208 28 139]/255;
    this_cols{2} = [241 182 218]/255;
    this_cols{3} = [77 172 38]/255;
    this_cols{4} = [184 255 134]/255;
    this_cols{5} = [67 162 202]/255;
    this_cols{6} = [.7 .7 .7];

    if 0
        % box plot
        figure('Position',[100 100 300 300])

        boxplot(rep_ves)
        xlim([0 7])
        ylim([0 80])
        ylabel('Var. Expl. (%)')
        set(gca,'XTick',1:6)
        set(gca,'XTickLabel',{'Motion 1','Motion 2','Avoid 1','Avoid 2','Tone','Residual'})
        xtickangle(45)

        h = findobj(gca,'Tag','Box');
        for j=1:length(h)
            patch(get(h(j),'XData'),get(h(j),'YData'),this_cols{7-j},'FaceAlpha',.6);
        end
    end
    
    % violin plot
    figure('Position',[100 100 300 300])
    hold on
    
    for d = 1:6
        this_ves = rep_ves(:,d);
        distributionPlot(this_ves,'color',[.9 .9 .9],'histOpt',1,'xValues',d,'showMM',0)
        % plot mean and CI
        sorted = sort(this_ves);
        if size(this_ves,1) ~= 80
            disp('Number of reps needs to be 80 for CIs')
        end
        ci_high = sorted(78);
        ci_low = sorted(3);    
        this_mean = mean(this_ves);
        plot(d,this_mean,'o','color',this_cols{d},'MarkerFaceColor',this_cols{d})
        errorbar(d,this_mean,this_mean-ci_low,this_mean-ci_high,'color',this_cols{d})
    end
    xlim([0 7])
    ylim([0 85])
    ylabel('Var. Expl. (%)')
    set(gca,'XTick',1:6)
    set(gca,'XTickLabel',{'Motion 1','Motion 2','Avoid 1','Avoid 2','Tone','Residual'})
    xtickangle(45)  
    
    if saveFigs
        fpath = fullfile(out_dir,'fig4','coding_dim_ves_iti_violin.svg');
        saveas(gca,fpath,'svg')
    end
    
    %% pie chart
    figure('Position',[100 100 300 300])
    h = pie(mean_total_ves,{'','','','','',''});
    patchHand = findobj(h, 'Type', 'Patch'); 
    for i = 1:6
        patchHand(i).FaceColor = this_cols{i};
    end

    if saveFigs
        fpath = fullfile(out_dir,'fig4','coding_dim_ves_iti_pie_chart.svg');
        saveas(gca,fpath,'svg')
    end
    
    %% print number for text
    
    % disp VE for 
    % dims 1 and 2 (motion)
    % dims 3 and 4 (avoid)
    
    % motion ve
    motion_ve = sum(rep_ves(:,1:2)')';
    sorted = sort(motion_ve);
    motion_ve_mn = mean(motion_ve);
    motion_ve_ci = sorted([3,78])';
    dim_ve_numbers_iti{1} = ['ITI motion VE: ' num2str(round(motion_ve_mn,1)) ' CI: [' num2str(round(motion_ve_ci(1),1)) ', ' num2str(round(motion_ve_ci(2),1)) ']'];

    % avoid ve
    avoid_ve = sum(rep_ves(:,3:4)')';
    sorted = sort(avoid_ve);
    avoid_ve_mn = mean(avoid_ve);
    avoid_ve_ci = sorted([3,78])';
    dim_ve_numbers_iti{2} = ['ITI avoid VE: ' num2str(round(avoid_ve_mn,1)) ' CI: [' num2str(round(avoid_ve_ci(1),1)) ', ' num2str(round(avoid_ve_ci(2),1)) ']'];

    cellfun(@(x) disp(x),dim_ve_numbers_iti)
    
end